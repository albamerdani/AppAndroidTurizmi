...
public class Resorte_malore extends AppCompatActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_resorte_malore);
    ...
    public void resorte_malore(){
        Statement st = null;
        Statement st2 = null;

        try{
            st = lidhje.createStatement();
            ResultSet rs = st.executeQuery("SELECT (Vendi) FROM turizmi WHERE Lloji = 'malor'");
            while(rs.next()){
